<?php 
	...
	p.a_Paterno as 'paterno', p.correo, p.telefono, c.nombre as 'categoria', d.nombre as 'deporte' FROM persona_categoria pc 
    INNER JOIN persona p ON pc.idPersona = p.id_Persona 
    INNER JOIN categoria c ON pc.idCategoria = c.idCategoria
    INNER JOIN deporte d ON c.id_deporte = d.idDeporte WHERE c.idCategoria =$_GET[id]");
