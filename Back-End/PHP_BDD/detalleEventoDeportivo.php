<?php 
	...
	INNER JOIN tipo_actividad ta ON act.tipo = ta.idTipo_Actividad WHERE act.idActividad = $_GET[id]");
    // $registros = mysqli_query($conexion,"SELECT act.idActividad,act.fecha,act.lugar,act.resultado,act.categoria,act.objetivo,dep.nombre, ta.descripcion 
	// FROM actividad act INNER JOIN deporte dep ON act.deporte = dep.idDeporte INNER JOIN tipo_actividad ta ON act.tipo = ta.idTipo_Actividad WHERE act.idActividad = $_GET[id]");
